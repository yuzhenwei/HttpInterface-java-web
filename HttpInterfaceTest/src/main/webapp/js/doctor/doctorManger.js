...
function deleteCaseList_doctor(){
	...
	var fronId=$("#caseListDlgTab_doctor").datagrid("getSelections");
        ...
        $.ajax({
 		   ...
 		   success: function(msg){
 			   if(msg.status == "ok"){
 				   ...
 				   $("#caseListDlgTab_doctor").datagrid('reload');
 			   }   
 		   }
 		});
...
function batchOutCase_doctor(){
	
	var interfaceName=$("#interfaceName_doctor").val();		
	window.location.href="LeadToExcel/outExcel.action?interfaceName="+interfaceName+"&region=Doctor";
}