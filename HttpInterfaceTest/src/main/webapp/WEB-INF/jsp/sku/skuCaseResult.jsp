...
<body>
	...
	<div style="padding:5px;" class="easyui-panel">
		<div style="padding:5px;">
			<select id="selectNameId_sku" class="easyui-combobox" name="dept" style="width:130px;" panelHeight=100>    
			    <option value="interfaceName">接口名称</option>   
			    <option value="authStatus">执行结果</option> 
			</select> 
			<input type="text" id="inputText_sku" name="inputText" /> 
			<a class="easyui-linkbutton" iconCls="icon-search" onClick="searchRunCaseResultInterface_sku()">查询</a>
		</div>		
			<table id="runCaseListDlgTab_sku" class="easyui-datagrid"  title="执行结果集列表" 
			pagination=false rownumbers=true  singleSelect=true resizable=true
			url="case/searchRunCaseResult.action?region=SKU" fitColumns=true nowrap=false toolbar="#deleteCaseListResultId_sku"
			>
		        <thead>
		            <tr>
		    ...
		    <div id="deleteCaseListResultId_sku" style="padding:5px;">
				<a class="easyui-linkbutton" iconCls="icon-cancel" onClick="deleteCaseListResult_sku()">删除执行结果</a>
		    </div>	
