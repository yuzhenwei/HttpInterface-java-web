...
<body>
	...
	<div style="padding:5px;" class="easyui-panel" fit=true >
		<div style="padding:15px 5px 5px 5px;">
			<label>接口名称</label>
			<input id="interfaceName_sku" name="interfaceName" class="easyui-textbox"></input>
			<a class="easyui-linkbutton" iconCls="icon-search" onClick="searchInterface_sku()">查询</a>
			<button class="easyui-linkbutton" iconCls="icon-cancel" onclick="clearInterface_sku()">清除</button>
		</div>
			<table id="caseListDlgTab_sku" class="easyui-datagrid"  title="用例集列表" 
			pagination=true rownumbers=true   resizable=true  url="case/selectCase.action?region=SKU" 
			toolbar="#caseListTb_sku,#edListTb_sku" fitColumns=true pageSize=15 pageList=[15,30]>
		        <thead>
		            <tr>
		...
		<div id="edListTb_sku" style="padding:5px;">
			...
			<!-- <label>请求头</label>
			<input id="headerIp_sku" class="easyui-combobox"  valueField="headerContent" textField="headerName" url="header/getHeaders.action" />  -->
			&nbsp;
			<a href="#" class="easyui-linkbutton" iconCls="icon-cut"  onClick="cutmuchCaseList_sku()">执行</a>
		</div>
	</div>	
