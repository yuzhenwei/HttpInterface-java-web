...
<!-- 	<script type="text/javascript"  src="js/jquery.min.js"></script>
 ...
 -->
	<%@include file="AddCasePage.jsp" %>
	<%@include file="EditCasePage.jsp" %>
	<title>用例管理</title>
</head>
<body>
	<!-- 用例集管理页面----------------------------------------------------------------------------------------------------------------------------------  -->
	...
	<div style="padding:5px;" class="easyui-panel" fit=true >
		<div style="padding:15px 5px 5px 5px;">
			<label>接口名称</label>
			<input id="interfaceName" name="interfaceName" class="easyui-textbox"></input>
			<a class="easyui-linkbutton" iconCls="icon-search" onClick="searchInterface()">查询</a>
			<button class="easyui-linkbutton" iconCls="icon-cancel" onclick="clearInterface()">清除</button>
		</div>
			<table id="caseListDlgTab" class="easyui-datagrid"  title="用例集列表" 
			pagination=true rownumbers=true   resizable=true  url="case/selectCase.action?region=OFC" 
			toolbar="#caseListTb,#edListTb" fitColumns=true pageSize=15 pageList=[15,30]>
		        <thead>
		            <tr>
		...
		<div id="edListTb" style="padding:5px;">
			...
			<label>请求头</label>
			<input id="headerIp_OFC" class="easyui-combobox"  valueField="headerContent" textField="headerName" url="header/getHeaders.action" /> 
			&nbsp;
			<a href="#" class="easyui-linkbutton" iconCls="icon-cut"  onClick="cutmuchCaseList()">执行</a>
		</div>
	</div>	
