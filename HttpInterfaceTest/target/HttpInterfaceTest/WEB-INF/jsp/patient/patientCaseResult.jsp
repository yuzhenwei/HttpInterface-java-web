...
<body>
	...
	<div style="padding:5px;" class="easyui-panel">
		<div style="padding:5px;">
			<select id="selectNameId_patient" class="easyui-combobox" name="dept" style="width:130px;" panelHeight=100>    
			    <option value="interfaceName">接口名称</option>   
			    <option value="authStatus">执行结果</option> 
			</select> 
			<input type="text" id="inputText_patient" name="inputText" /> 
			<a class="easyui-linkbutton" iconCls="icon-search" onClick="searchRunCaseResultInterface_patient()">查询</a>
		</div>		
			<table id="runCaseListDlgTab_patient" class="easyui-datagrid"  title="执行结果集列表" 
			pagination=false rownumbers=true  singleSelect=true resizable=true
			url="case/searchRunCaseResult.action?region=patient" fitColumns=true nowrap=false toolbar="#deleteCaseListResultId_patient"
			>
		        <thead>
		            <tr>
		    ...
		    <div id="deleteCaseListResultId_patient" style="padding:5px;">
				<a class="easyui-linkbutton" iconCls="icon-cancel" onClick="deleteCaseListResult_patient()">删除执行结果</a>
		    </div>	
